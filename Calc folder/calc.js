//Created by Github: @vinyl Davyl

//Inspired by John Wells and Samuel Kamulanje for helpful suggestions and improvement
...
//To control what happens when PI and Euler is clicked
function PE(b){
    decimalPoint = ".";
    dotCounter = 15;
    var cons = b;
...
//When sqr, cube e.t.c are clicked, this function, math(), does the following: 1. It extract the last number you entered from variable 'first' and keeps it as variable 'firstI'. 2. It calls mathematics function to work on the variable 'firstI'. The answer is kept in 'result' variable. 3. It displays the answer of the other previous numbers entered and the 'result' from the mathemtics e.g. 5 + 4 which was 3 + 2 + 2^2. Here 3 + 2 has become 5 and because of prep() function.
function maths(a){
    math = a;
    try{
        if (operatorSign == "+"){
...
function digit(b){
    ...
    entered = b;
    if (rootNpower_Sign != ""){
        first = (first === "0" && entered !== ".") ? entered: first + second;
...
//When operators ,+ - / * are clicked op() function does a number of things. It checks if power or root were clicked.If yes, it calculates the previous number and then add the new operator. If not, it adds the new operator clicked.
function operators(b){
 ...
 return eval(second);
    ...
    document.getElementById("dispaly1").innerHTML = first;
        return (second == undefined) ? 0:
        eval(second);
}
    } else{
        operatorSign = b;
